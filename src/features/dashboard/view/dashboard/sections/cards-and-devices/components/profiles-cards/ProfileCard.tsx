...
import {
  cardBackStyle,
  StyledDefaultProfileIcon,
  StyledFlippableCard,
} from './ProfileCard.styles';

export type ProfileCardProps = Required<
  VariantProps<typeof StyledFlippableCard>
> & {
  title: string;
  isDefault?: boolean;
};

const DefaultProfileSvgIcon = () => (
  <StyledDefaultProfileIcon>
    <svg
      width="22"
      height="20"
...
const ProfileCard = ({ title, isDefault, type }: ProfileCardProps) => (
  <StyledFlippableCard
    type={type}
    frontContent={
      <>
        {isDefault && <DefaultProfileSvgIcon />}
        <span>{title}</span>
      </>
    }
